...
- 🟠 **Deeper engine** — item stacking *buffs* (Berserker/Glass Cannon), active items as explicit combo steps, fuller melee (the +50% weapon scaling, melee items, separate melee-resist channel), and procs counted in sustained DPS (currently burst-only).
