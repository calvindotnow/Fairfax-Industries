...
- 🟠 **Deeper engine** — active items as explicit *damage* combo steps (their on-cast self-buffs are modeled, but an active that deals direct damage isn't folded into the burst sequence yet); recompute imbued ability numbers (the imbue relationship is shown but the ability's stats aren't recomputed); and the stacking stats not yet modeled (spirit-damage amp like Escalating Exposure, heal-per-stack like Restorative Locket — sliders show but are display-only). (Already modeled: on-hit procs, range/fire-rate conditionals, resist debuffs, weapon/fire-rate/health/sprint stacking, active self-buffs, accuracy-scaled sustained DPS, and fuller melee — melee-damage items + 50% weapon scaling + a melee-resist channel.)
